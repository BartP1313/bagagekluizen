...
def menu():
    ...
    keuze = int(input(f"Wat wil je kiezen? (1-4) "))

    try:
        keuze = int(keuze)
        if keuze < 1 or keuze > 4:
            print("Ongeldige keuze, voer een getal in tussen 1 en 4.")
            return menu()
        return keuze
    except ValueError:
...
def aantal_kluizen_vrij():
    try:
        with open("kluizen.txt", "r") as bestand:
            kluisregels = bestand.readlines()
        aantal_bezette_kluizen = len(kluisregels)
        aantal_vrije_kluizen = 12 - aantal_bezette_kluizen
        return aantal_vrije_kluizen
    except FileNotFoundError:
        print("Fout: Het bestand 'kluizen.txt' is niet gevonden.")
        return 12
    ...
    try:
        with open("kluizen.txt", "r") as bestand:
            for regel in bestand.readlines():
                ...
                bezette_kluizen.append(kluisnummer)

        beschikbare_kluizen = [kluis for kluis in alle_kluizen if kluis not in bezette_kluizen]

        if len(beschikbare_kluizen) == 0:
            return -2

        kluiscode = input("Voer een kluiscode van minimaal 4 tekens in: ")
        if len(kluiscode) < 4:
            return -1

        nieuwe_kluisnummer = beschikbare_kluizen[0]
        with open("kluizen.txt", "a") as bestand:
            bestand.write(f"{nieuwe_kluisnummer};{kluiscode}\n")
...
def development_code():
    while True:
        if keuze == 1:
            ...
            print(f"Er zijn {aantal_vrij} kluizen beschikbaar.")
        elif keuze == 2:
            resultaat = nieuwe_kluis()
            if resultaat == -1:
                print("Fout: Ongeldige kluiscode.")
            elif resultaat == -2:
                print("Fout: Geen kluizen meer vrij.")
            else:
                print(f"U heeft kluisnummer {resultaat} gekregen.")
        elif keuze == 3:
            if kluis_openen():
                print("Kluis geopend.")
            else:
                print("Fout: Onjuiste kluisnummer of kluiscode.")
        elif keuze == 4:
            if kluis_teruggeven():
